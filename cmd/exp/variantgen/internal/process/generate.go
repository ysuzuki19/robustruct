...
func Generate(args GenerateArgs) ([]byte, error) {
	...
	name := args.Name
	enumDefName := fmt.Sprintf("%s%s", coder.Capitalize(args.Name), coder.Bracket(defTypeParams))
	enumUseName := fmt.Sprintf("%s%s", coder.Capitalize(args.Name), coder.Bracket(useTypeParams))
